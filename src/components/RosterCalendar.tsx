    ...
    <div className="space-y-6">
      {groupedSchedule.map(({ date, timeSlots }) => {
        ...
        return (
          ...
          >
            {/* Day Header */}
            <div className="bg-[#0b3f7a] text-white px-5 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <CalendarIcon className="w-4 h-4 text-slate-400" />
                <h3 className="text-sm font-bold tracking-wide">{formattedDate}</h3>
                <span className="text-[11px] bg-[#0d4b8f] text-blue-100 px-2 py-0.5 rounded font-mono">
                  {date}
                </span>
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {(venues || []).map((vItem, vIdx) => {
                      ...
                      return (
                        <div
                          key={vIdx}
                          className="bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-2.5 hover:border-blue-300 transition-all"
                        >
                          {/* Venue & Course title */}
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="font-black text-xs text-slate-900 flex items-center gap-1.5">
                                <span className="bg-blue-100 text-slate-900 px-1.5 py-0.2 rounded font-mono text-[11px]">
                                  {vItem.course?.code}
                                </span>
                            ...
                            <div className="text-right">
                              <span className="text-[10px] text-slate-700 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200 font-medium">
                                ...
                                >
                                  <div>
                                    ...
                                    <div className="flex items-center gap-1 mt-1">
                                      {chief.status === 'suggested' && (
                                        <span className="text-[9px] px-1 py-0.2 bg-slate-50 text-slate-600 rounded border border-slate-200">
                                          Suggested
                                        </span>
                                      )}
                                      {chief.status === 'approved' && (
                                        <span className="text-[9px] px-1 py-0.2 bg-blue-50 text-[#0d4b8f] rounded border border-blue-200 font-semibold">
                                          Approved
                                        </span>
                                      )}
                                      {chief.status === 'edited' && (
                                  ...
                                  <div className="flex items-center gap-1">
                                    <button
                                      ...
                                      className="p-1 rounded text-slate-500 hover:text-[#0d4b8f] hover:bg-slate-100"
                                    >
                                    ...
                                    {chief.status !== 'approved' && (
                                      <button
                                        ...
                                        title="Approve"
                                        className="p-1 rounded text-[#0d4b8f] hover:bg-blue-50"
                                      >
